...
foreach($source as $one)
{
    ...
    $bodyFiltered["poster"] = $conf["photobankBackendHost"]."/core/cache/plugins/imageviewer/".$fileSource['id']."/".$fileSource['unique_hash']."/585x440_cropped.jpg";

    $bodyFiltered["date_taken"] = date("d/m/Y", strtotime($one["date_taken"]));
    $bodyFiltered["date_created"] = date("d/m/Y", strtotime($body["DateCreated"]));
